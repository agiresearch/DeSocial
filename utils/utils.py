...
class NeighborSampler:
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
        ...
        for node_idx, per_node_neighbors in enumerate(adj_list):
            ...
            sorted_per_node_neighbors = sorted(per_node_neighbors, key=lambda x: x[1])
            ...
            self.nodes_neighbor_times.append(np.array([x[1] for x in sorted_per_node_neighbors]))

            # additional for time interval aware sampling strategy (proposed in CAWN paper)
            if self.sample_neighbor_strategy == 'time_interval_aware':
                self.nodes_neighbor_sampled_probabilities.append(self.compute_sampled_probabilities(np.array([x[1] for x in sorted_per_node_neighbors])))
    ...
    def find_neighbors_before(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        ...
        #print(len(self.nodes_neighbor_times), node_id)
        i = np.searchsorted(self.nodes_neighbor_times[node_id], interact_time)

        if return_sampled_probabilities:
            return self.nodes_neighbor_ids[node_id][:i], self.nodes_neighbor_times[node_id][:i], \
    ...
    def get_multi_hop_neighbors(self, num_hops: int, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        ...
        nodes_neighbor_ids, nodes_neighbor_times = self.get_historical_neighbors(node_ids=node_ids,
        ...
        for hop in range(1, num_hops):
            ...
            nodes_neighbor_ids, nodes_neighbor_times = self.get_historical_neighbors(node_ids=nodes_neighbor_ids_list[-1].flatten(),
        ...
        return nodes_neighbor_ids_list, nodes_neighbor_times_list
...
def get_neighbor_sampler(data: Data, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
    ...
    for src_node_id, dst_node_id, node_interact_time in zip(data.src_node_ids, data.dst_node_ids, data.node_interact_times):
        adj_list[src_node_id].append((dst_node_id, node_interact_time))
        adj_list[dst_node_id].append((src_node_id, node_interact_time))
...
class NegativeEdgeSampler(object):
    ...
    def sample(self, size: int, batch_src_node_ids: np.ndarray = None, batch_dst_node_ids: np.ndarray = None,
        ...
        elif self.negative_sample_strategy == 'random_with_collision_check':
            negative_src_node_ids, negative_dst_node_ids = self.random_sample_with_collision_check(size=size,
